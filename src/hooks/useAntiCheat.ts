...
export function useAntiCheat({
    ...
    const [maxViolations, setMaxViolations] = useState(parseInt(process.env.NEXT_PUBLIC_MAX_WARNINGS || '15', 10));
        ...
        fetch(`/api/attempts/${attemptId}/violation`)
            ...
            .then(data => {
                if (isMounted.current && data.count !== undefined) {
                    ...
                    setMaxViolations(data.maxViolations || parseInt(process.env.NEXT_PUBLIC_MAX_WARNINGS || '15', 10));
                }
            })
            .catch(console.error);
